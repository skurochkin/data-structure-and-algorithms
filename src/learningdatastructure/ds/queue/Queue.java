...
public class Queue {
    ...
    private int maxSize; // initializes the set number of the slots
    ...
    public long remove(){ // remove item from the front of the queue
        ...
        front++;
        if(front == maxSize){
            front = 0; // we can set front back to the 0th index so that we can utiliize the entire array again
        }
        nItems--;
        return temp;
    }
